...
| H4 | Retry-машина — единый таймер с декрементом попыток, ретрай на ошибку `write` | ✅ `dq-dsp-ui/src/hooks/useWebSerial.ts` (общий бюджет 4 попытки, единый таймер) |
| H3 | Bulk-континуация — при активном `bulkRxBufferRef` не разбирать сигнатуры ACK/ERROR/BULK | ✅ `dq-dsp-ui/src/hooks/useWebSerial.ts` (все фреймы → continuation, ACK/ERROR только для in-flight) |
