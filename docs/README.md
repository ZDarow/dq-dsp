...
| Документ | О чём |
|---|---|
| [`DOCUMENTATION.md`](DOCUMENTATION.md) | Полное руководство: архитектура, установка, API, сценарии использования, troubleshooting и FAQ. Рекомендуется начать здесь. |
...
- **Прозрачность протокола**: `serial_protocol.h` (C) — канонический источник; TS-зеркало в `dq-dsp-ui/src/types/` обязано быть в синхроне.
