...
bool tud_audio_set_itf_close_EP_cb(uint8_t rhport, tusb_control_request_t const *p_request)
{
    ...
    if (s_uac_device->spk_itf_num == itf && alt == 0) {
        ESP_LOGI(TAG, "Speaker interface %d closed (alt=0)", itf);
...
bool tud_audio_set_itf_cb(uint8_t rhport, tusb_control_request_t const *p_request)
{
    ...
    if (s_uac_device->spk_itf_num == itf && alt != 0) {
        ...
        ESP_LOGI(TAG, "Speaker interface %d-%d opened (resolution=%d bytes_per_ms=%u sr=%lu)",
                 itf, alt, s_uac_device->spk_resolution,
                 (unsigned)s_uac_device->spk_bytes_per_ms,
                 (unsigned long)s_uac_device->current_sample_rate);
    }
#endif
