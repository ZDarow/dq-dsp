...
import type { BLEMessageType, BLEBlockType, BLEParamType, BLEStatusCode } from './ble-protocol'
...
export function encodeSerialFrame(payload: Uint8Array): Uint8Array {
  if (payload.length > SERIAL_MAX_PAYLOAD_SIZE) {
    throw new RangeError(`Payload size ${payload.length} exceeds max ${SERIAL_MAX_PAYLOAD_SIZE}`)
  }
