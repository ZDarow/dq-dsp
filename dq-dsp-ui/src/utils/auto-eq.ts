 ...
 */
function isLikelyExcessPhase(residual: number[], peakIdx: number, frequencies: number[]): boolean {
...
export function computeAutoEQ(
  ...
  for (let n = 0; n < params.numBands; n++) {
    ...
    const maxQForFreq = peakFreq > TRANSITION_END_HZ ? Math.min(params.maxQ, 3) : params.maxQ
