...
const NAV_ITEMS: {
  ...
  {
    block: { type: 'input', index: 0 },
    labelKey: 'nav.input',
    labelParams: { n: 1 },
    color: INPUT_COLORS[0],
  },
  {
    block: { type: 'input', index: 1 },
    labelKey: 'nav.input',
    labelParams: { n: 2 },
  ...
  {
    block: { type: 'output', index: 0 },
    labelKey: 'nav.output',
    labelParams: { n: 1 },
    color: OUTPUT_COLORS[0],
  },
  {
    block: { type: 'output', index: 1 },
    labelKey: 'nav.output',
    labelParams: { n: 2 },
    color: OUTPUT_COLORS[1],
  },
  {
    block: { type: 'output', index: 2 },
    labelKey: 'nav.output',
    labelParams: { n: 3 },
    color: OUTPUT_COLORS[2],
  },
  {
    block: { type: 'output', index: 3 },
    labelKey: 'nav.output',
    labelParams: { n: 4 },
    color: OUTPUT_COLORS[3],
  },
]
      ...
      {NAV_ITEMS.map((item, i) => {
        ...
        return (
          <div key={i} className="flex items-center">
            ...
            {linked && item.block?.type === 'input' && item.block.index === 1 && <LinkIndicator />}
            {linked &&
              item.block?.type === 'output' &&
              shouldDrawLinkIconBefore(item.block.index) && <LinkIndicator />}
            <button
