...
export function AllChannelsResponseChart() {
  ...
  const inputMagnitudes = useMemo(() => {
    return inputs.map((inp, i) => {
      ...
      const roomResp = roomEqEnabled ? eqBandsResponse(roomEqBands[i], frequencies, sampleRate) : null;
  ...
  const toX = (freq: number) => freqToLogPosition(freq, FREQUENCY_RANGE.min, FREQUENCY_RANGE.max) * plotW;
  const toY = (db: number) => plotH * (1 - (db + DB_RANGE) / (2 * DB_RANGE));

  const toggleFixed = (i: number) =>
    setFixedVisible((v) => { const n = [...v]; n[i] = !n[i]; return n; });
