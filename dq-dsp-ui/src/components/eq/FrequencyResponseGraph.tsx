  ...
  return (
    ...
    >
      ...
      {GRID_FREQUENCIES.map((f) => {
        ...
        return (
          <g key={`f-${f}`}>
            <line x1={x} y1={PAD.top} x2={x} y2={PAD.top + PLOT_H} stroke="var(--color-surface-bg)" strokeWidth={0.5} />
            <text x={x} y={GRAPH_H - 4} textAnchor="middle" fill="var(--color-text-dimmed)" fontSize={8}>
              {f >= 1000 ? `${f / 1000}k` : f}
            </text>
          </g>
        );
      ...
      {GRID_DB.map((db) => {
        ...
        return (
          <g key={`db-${db}`}>
            ...
            />
            <text x={PAD.left - 4} y={y + 3} textAnchor="end" fill="var(--color-text-dimmed)" fontSize={8}>
              {db > 0 ? `+${db}` : db}
            </text>
          </g>
        );
