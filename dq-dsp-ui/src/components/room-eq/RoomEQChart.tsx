...
export function RoomEQChart({ inputIndex }: RoomEQChartProps) {
  ...
  const targetValues = useMemo(() => {
    switch (roomTargetCurve) {
      case 'harman':
        return harmanTarget(frequencies)
      case 'tilt':
        return tiltTarget(frequencies, roomTiltSlope)
      default:
        return flatTarget(frequencies)
    }
  ...
  const predictedPath = useMemo(() => {
    if (!smoothedMeasurement || smoothedMeasurement.length === 0) return ''
    return buildPredictedPath(
      smoothedMeasurement,
      eqResponse.map((p) => p.magnitude),
      frequencies,
      toX,
      toY,
    )
  ...
  const phaseToY = useCallback((deg: number) => plotH * (1 - (deg + 180) / 360), [plotH])
  ...
  const cursorInfo = useMemo(() => {
    ...
    const freqIdx = frequencies.reduce(
      (best, f, i) => (Math.abs(f - freq) < Math.abs(frequencies[best] - freq) ? i : best),
      0,
    )
      ...
      <svg width={plotW} height={plotH} className="absolute top-0 left-0">
        ...
        {measurementPath && (
          <path
            d={measurementPath}
            fill="none"
            stroke="var(--color-chart-measurement)"
            strokeWidth={1.5}
            opacity={0.5}
            clipPath="url(#room-eq-clip)"
          />
      ...
      <div className="absolute top-1 right-10 flex gap-3 text-xs items-center">
        ...
        <button
          ...
          style={{
            ...
            backgroundColor: showPhase
              ? 'color-mix(in srgb, var(--color-chart-phase) 15%, transparent)'
              : 'transparent',
        ...
        <>
          ...
          >
            ...
            {cursorInfo.measDb !== null && (
              <div style={{ color: 'var(--color-chart-measurement)' }}>
                Meas: {cursorInfo.measDb.toFixed(1)} dB
              </div>
            )}
            <div style={{ color: 'var(--color-chart-eq)' }}>
              EQ: {cursorInfo.eqDb > 0 ? '+' : ''}
              {cursorInfo.eqDb.toFixed(1)} dB
            </div>
            {showPhase && (
              <div style={{ color: 'var(--color-chart-phase)' }}>
                Phase: {cursorInfo.eqPhase > 0 ? '+' : ''}
                {cursorInfo.eqPhase.toFixed(1)}°
              </div>
            )}
            {cursorInfo.predictedDb !== null && (
              <div style={{ color: 'var(--color-chart-predicted)' }}>
                Pred: {cursorInfo.predictedDb.toFixed(1)} dB
              </div>
            )}
            {cursorInfo.targetDb !== null && (
              <div style={{ color: 'var(--color-chart-target)' }}>
                Target: {cursorInfo.targetDb.toFixed(1)} dB
              </div>
            )}
          </div>
        </>
...
function interpolateMeasurement(measurement: MeasurementPoint[], freq: number): number {
  ...
  if (freq >= measurement[measurement.length - 1].frequency)
    return measurement[measurement.length - 1].magnitude
