...
export function DriftChart({ history }: DriftChartProps) {
  ...
  const fillStatusColor = idle
    ? 'var(--color-text-dimmed)'
    : !empty && (fill < 15 || fill > 85)
      ? COLOR_DANGER
      : !empty && (fill < 25 || fill > 75)
        ? COLOR_WARN
        : COLOR_FILL
      ...
      <div className="flex items-center gap-1.5 px-2 py-1.5 flex-wrap">
        ...
        <span className="pill-badge" style={{ ['--pill-color' as string]: fillStatusColor }}>
          <span
            ...
            style={{ backgroundColor: fillStatusColor, marginRight: 0 }}
          />
        ...
        <span className="pill-badge" style={{ ['--pill-color' as string]: COLOR_PPM }}>
          ...
          <span className="value-mono">
            {empty ? '--' : `${ppm >= 0 ? '+' : ''}${ppm.toFixed(1)}`}
          </span>
        </span>
        <span
          className="pill-badge"
          style={{ ['--pill-color' as string]: COLOR_TARGET, opacity: 0.65 }}
        >
  ...
  return (
    <div className="relative" style={{ height: CHART_HEIGHT }}>
      ...
      <div className="absolute inset-0 pointer-events-none font-mono text-[0.65rem] font-semibold leading-none">
        ...
        {/* X-axis time labels */}
        <span className="absolute pl-0.5 text-text-secondary" style={{ left: 0, bottom: 2 }}>
          -60s
        </span>
        <span
        ...
        <span className="absolute pr-0.5 text-text-secondary" style={{ right: 0, bottom: 2 }}>
          now
        </span>
      </div>
    </div>
