...
export function JitterChart({ history }: JitterChartProps) {
  ...
  const deadlineUs = latest && latest.blocksProcessed > 0 ? 1_000_000 / latest.blocksProcessed : 0
  const peakLoad = !empty && deadlineUs > 0 ? latest!.dspMaxUs / deadlineUs : 0
  const loadStatusColor = peakLoad >= 0.9 ? COLOR_DANGER : peakLoad >= 0.6 ? COLOR_WARN : COLOR_AVG
      ...
      <div className="flex items-center gap-1.5 px-2 py-1.5 flex-wrap">
        ...
        <span className="pill-badge" style={{ ['--pill-color' as string]: loadStatusColor }}>
          <span
            ...
            style={{ backgroundColor: loadStatusColor, marginRight: 0 }}
          />
          Avg <span className="value-mono">{empty ? '--' : `${loadPct}%`}</span>
        </span>
        <span
          className="pill-badge"
          style={{ ['--pill-color' as string]: COLOR_BAND, opacity: 0.7 }}
        >
          <span className="status-dot" style={{ backgroundColor: COLOR_BAND, marginRight: 0 }} />
          Min/Max{' '}
          <span className="value-mono">
            {empty ? '--' : `${latest!.dspMinUs}/${latest!.dspMaxUs}us`}
          </span>
        </span>
        <span
          className="pill-badge"
          style={{ ['--pill-color' as string]: COLOR_DANGER, opacity: 0.65 }}
        >
...
function JitterPlot({ history, deadlineUs }: JitterPlotProps) {
  ...
  const bandBottom = history
    .map((d, i) => `${toX(i)},${toY(d.dspMinUs)}`)
    .reverse()
    .join(' ')
  const bandPoints = `${bandTop} ${bandBottom}`

  const avgPath = history
    .map((d, i) => `${i === 0 ? 'M' : 'L'}${toX(i)},${toY(d.dspAvgUs)}`)
    .join(' ')
  const maxPath = history
    .map((d, i) => `${i === 0 ? 'M' : 'L'}${toX(i)},${toY(d.dspMaxUs)}`)
    .join(' ')
  const minPath = history
    .map((d, i) => `${i === 0 ? 'M' : 'L'}${toX(i)},${toY(d.dspMinUs)}`)
    .join(' ')
  ...
  return (
    <div className="relative" style={{ height: CHART_HEIGHT }}>
      ...
      <div className="absolute inset-0 pointer-events-none font-mono text-[0.65rem] font-semibold leading-none">
        ...
        <span className="absolute pl-1 text-text-secondary" style={{ left: 0, bottom: 2 }}>
          -60s
        </span>
        <span
        ...
        <span className="absolute pr-1 text-text-secondary" style={{ right: 0, bottom: 2 }}>
          now
        </span>
      </div>
    </div>
