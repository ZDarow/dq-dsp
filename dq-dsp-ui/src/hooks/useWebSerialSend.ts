...
export function useWebSerialSend(callbacks: SendCallbacks) {
  ...
  const handleAck = useCallback(
    (payload: Uint8Array) => {
      ...
      const statusMsg: BLEAckMsg | BLEErrorMsg =
        ackMsgType === BLE_MSG_ERROR
          ? {
              msgId,
              msgType: BLE_MSG_ERROR,
              statusCode,
              detail: payload.length >= 4 ? payload[3] : 0,
            }
  ...
  return {
    ...
    get queueSize() {
      return sendQueueRef.current.length
    },
  }
}
