...
export function createSerialMiddleware(
  store: { subscribe: (listener: (state: DSPStore, prev: DSPStore) => void) => () => void; getState: () => DSPStore },
  send: SendFn,
): () => void {
  ...
  function diffEQBands(
    ...
    for (let b = 0; b < curr.length; b++) {
      ...
      if (p.q !== c.q) {
        sendDebounced(`eq-${block}-${channel}-${b}-q`, () =>
          encodeEQBandQ(block, channel, b, c.q),
        );
      }
  ...
  function diffRoomEQBands(
    ...
    for (let b = 0; b < curr.length; b++) {
      ...
      if (p.q !== c.q) {
        sendDebounced(`room-eq-${channel}-${b}-q`, () =>
          encodeRoomEQBandQ(channel, b, c.q),
        );
      }
      if (p.filterType !== c.filterType) {
