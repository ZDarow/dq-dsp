  ...
  return (
    <div className="flex items-center justify-between px-4 py-2 border-b border-surface-bg/40 flex-wrap gap-y-2 gap-x-4">
      ...
      <div className="flex items-center gap-4">
        ...
        >
          <label className="section-label" style={{ fontSize: '0.65rem' }}>{t('toolbar.master')}</label>
          <Tooltip content={t('toolbar.masterTooltip', { value: formatDb(masterVolume) })} wrapperClassName="inline-block">
            <input
              type="range"
              min={-72}
              max={12}
        ...
        <Tooltip content={t('toolbar.sampleRateTooltip')}>
          <div
            ...
            style={{ borderRadius: 'var(--radius-panel)' }}
          >
            <label className="section-label" style={{ fontSize: '0.65rem' }}>{t('toolbar.sampleRate')}</label>
      ...
      <div className="flex items-center gap-4">
        <SerialStatusBar />
        <Tooltip content={t('toolbar.themeTooltip', { theme: theme === 'dark' ? 'light' : 'dark' })}>
          <button
            onClick={toggleTheme}
